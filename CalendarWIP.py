...
from kivy.properties import BoundedNumericProperty, ObjectProperty, partial
...
class CalendarLessThan30Days(Widget):
	...
	dayBarLayout = RelativeLayout(size_hint_y=.25)  # Layout for the dayBar, if it exists, and body
	...
	def __init__(self, **kwargs):
		...
		for i in range(24):
			...
			lbl.bind(text_size=lambda inst,text_size: print(inst.text,text_size))
		...
		if self.days > 1:  # Add the hour bar if it's not one day
			...
			self.dayBarLayout.add_widget(
				Image(source="CalendarActive.png", allow_stretch=True, keep_ratio=False,
				      color=(1, 0, 0, 1)))  # Test Widget
		for i in range(self.days):
			...
			event = Event(size_hint_y=float(self.eventHeight) / self.bodyLayout.height, x=1,
			              pos_hint={"center_y": 1 - (150./2048) - timeToPos(datetime.now())},
